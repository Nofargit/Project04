...
app.get('/all_products', (req,res)=>{
    ...
    const con = global.Application.get('CONNECTION');
    const sql = ` SELECT products.product_id, products.product_name, products.category_id, products.price, products.product_picture , product_category.category_name FROM products
...
app.get('/select_items_from_cart/:cartID', ( req, res )=>{
    ...
    const con = global.Application.get('CONNECTION');
        const sql = `SELECT products.product_id, products.price, products.product_name, products.product_picture, product_category.category_name, cart_item.quantity, cart_item.total FROM cart_item
        INNER JOIN products
        ON cart_item.product_id = products.product_id
        INNER JOIN product_category
        ON cart_item.category_id = product_category.category_id
...
app.put('/update_quantity_on_cart' , ( req, res )=>{
    ...
    const con = global.Application.get('CONNECTION');
    const sql = `UPDATE cart_item SET quantity= ${quantity} WHERE product_id = ${product_id} `
    con.query(sql, (err, result, fields) => {
        if (err) {
            res.json(err)
        }
...
app.post('/down_order' , (req,res) =>{
     const con = global.Application.get('CONNECTION');
    const sql = ` SELECT products.product_id, products.price, products.product_name, products.product_picture, product_category.category_name, cart_item.quantity, cart_item.total , orders.order_id ,orders.total_price FROM cart_item
    INNER JOIN products
    ON cart_item.product_id = products.product_id
    INNER JOIN product_category
    ON cart_item.category_id = product_category.category_id
    ...
    ORDER BY product_category.category_name
    `
        con.query(sql, (err, result, fields) => {
        if (err) {
            res.json(err)
